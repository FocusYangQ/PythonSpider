...
QQNUMBER = "1098437473@qq.com"
PASSWORD = "Yangqing_123"
...
ROOTPATH = "/Users/yangqing/Desktop/QQe-mailDownload"
...
MAIL_SELECTOR = {
    ...
    'create_tag_setting_xpath': ["//a[starts-with(#id,'folder_tag_') and contains(#id, '_name')]",
                                 "//tr[starts-with(#id,'tag_')]/td[1]/div[2]/a"],
...
def init_prettytable():
    ...
    PRETTY_TABLE['attach_list'].field_names = ['count', 'filename', 'index', 'name', 'title', 'email', 'fileindex',
                                               'filebyte', 'filetype', 'page', 'timeout', 'timestamp']
    PRETTY_TABLE['attach_list'].align = 'l'

    # 过期附件
    PRETTY_TABLE['attach_timeout'] = prettytable.PrettyTable()
    PRETTY_TABLE['attach_timeout'].field_names = ['count', 'filename', 'index', 'name', 'title', 'email', 'fileindex',
                                                  'filebyte', 'filetype', 'page', 'timeout', 'timestamp']
    ...
    if ATTACH_BACKLIST_FILETYPE != [''] or ATTACH_WHITELIST_FILETYPE != ['']:
        ...
        PRETTY_TABLE['attach_backlist'].field_names = ['count', 'filename', 'index', 'name', 'title', 'email',
                                                       'fileindex', 'filebyte', 'filetype', 'page', 'timeout',
                                                       'timestamp']
...
def init_webdriver():
    ...
    options.add_argument(
        [f'--user-data-dir={USERDATA_FOLDER}', f'--user-data-dir {USERDATA_FOLDER}'][bool(is_mac_user)])
...
def check_tag_exists_in_list():
    ...
    # 读取列表
    tags = [re.sub(r' 未读邮件(.*?)封', '', tag.web_element.get_attribute('title')) for tag in
            find_all(S('#tagfolders > li > a'))]
...
def update_folder_info():
    goto(
        f"https://{LOCALDATA['token_domin']}.qq.com/cgi-bin/mail_list?folderid={LOCALDATA['folder_id']}&page={LOCALDATA['token_page']}&sid={LOCALDATA['token_sid']}&nocheckframe=true")
...
def init_folder_task():
    ...
    if PAGES_TASK['start'] > 1 and TITLE_TASK['start'] < (PAGES_TASK['start'] * LOCALDATA['show_count']):
        xprint(
            f"{C.BGPURPLE}[参数异常]{C.END} TITLE_TASK[start] 不能小于 {C.GREEN}{PAGES_TASK['start'] * LOCALDATA['show_count']}{C.END} ({PAGES_TASK['start']} * {LOCALDATA['show_count']})")
    ...
    if PAGES_TASK['start'] > 1 and TITLE_TASK['start'] < (PAGES_TASK['start'] * LOCALDATA['show_count']):
        ...
        xprint(
            f"{C.GREEN}[自动修正]{C.END} PAGES_TASK[start] = {PAGES_TASK['start']}\t{C.GOLD}即将跳转至第{int(LOCALDATA['token_page']) + 1}页。{C.END}")
        ...
        xprint(
            f"{C.GREEN}[自动修正]{C.END} TITLE_TASK[start] = {PAGES_TASK['start']}\t第 {TITLE_TASK['start']} 封邮件位于第 {int(LOCALDATA['token_page']) + 1} 页 (当前每页显示 {LOCALDATA['show_count']} 封)")
    else:
    ...
    if LOCALDATA['token_page'] >= 1:
        ...
        xprint(
            f"{C.GREEN}[自动修正]{C.END} 由于 LOCALDATA['token_page'] = {PAGES_TASK['token_page']}\t即将从第{C.GREEN}{LOCALDATA['title_index']}{C.END} 封邮件开始计数。")
        goto(
            f"https://{LOCALDATA['token_domin']}.qq.com/cgi-bin/mail_list?folderid={LOCALDATA['folder_id']}&page={LOCALDATA['token_page']}&sid={LOCALDATA['token_sid']}&nocheckframe=true")
...
def load_folder_title():
    ...
    for i, e in enumerate(maillist, start=0):
        title = {}
        title.update({'index': TITLE_TASK['relay'] + i if not bool(PAGES_TASK['reverse']) else TITLE_TASK['relay'] +
                                                                                               LOCALDATA[
                                                                                                   'folder_title'] - len(
            LOCALDATA['title_list'])})
        ...
        if TITLE_BACKLIST_KEYS != [''] and any([key in namelist[i] for key in TITLE_BACKLIST_KEYS]):
            ...
            if bool(can_print_prettytable): PRETTY_TABLE['title_backlist'].add_row(
                [title['index'], title['page'], title['name'], title['title'], title['email'],
                 time.strftime('%Y-%m-%d %H:%M:%S', title['timestamp'])])
            continue
        ...
        if bool(can_print_prettytable):
            PRETTY_TABLE['title_list'].add_row(
                [title['index'], title['page'], title['name'], title['title'], title['email'],
                 time.strftime('%Y-%m-%d %H:%M:%S', title['timestamp'])])
        ...
        if bool(can_print_title):
            xprint(
                f"+ {title['index']:<4}: {title['page']}\t{title['email']:<24}\t{title['name']:<24}\t{title['title']}")
        ...
        if TITLE_TASK['step'] >= 1 and len(LOCALDATA['title_list']) > TITLE_TASK['start'] + TITLE_TASK['step']:
            xprint(
                f"{C.BGPURPLE}[TITLE_TASK: step]{C.END} 任务结束。{TITLE_TASK['start']}(+{TITLE_TASK['step']}) -> {len(LOCALDATA['title_list'])}")
            PAGES_TASK['autoNext'] = 0
            break
...
def foreach_folder_title():
    ...
    while bool(can_next_page) and LOCALDATA['token_page'] < LOCALDATA['max_page']:
        ...
        if not bool(PAGES_TASK['autoNext']):
            xprint(
                f"{C.BGPURPLE}[PAGES_TASK: autoNext]{C.END} 任务结束。{LOCALDATA['token_page'] + 1} / {LOCALDATA['max_page']}")
            break

        # PAGES_TASK['end']
        if PAGES_TASK['end'] >= 1 and int(LOCALDATA['token_page']) + 1 > PAGES_TASK['end']:
            xprint(
                f"{C.BGPURPLE}[PAGES_TASK: end]{C.END} 任务结束。{int(LOCALDATA['token_page']) + 1} / {LOCALDATA['max_page']}")
            break
        ...
        nextpage = int(next_btn.get_attribute("page")) if not bool(is_exmail_user) else int(
            get_querystring(next_btn.get_attribute("href"))['page'])

        # PAGES_TASK['step']
        if PAGES_TASK['step'] >= 1 and nextpage < PAGES_TASK['start'] + PAGES_TASK['step']:
            xprint(
                f"{C.BGPURPLE}[PAGES_TASK: step]{C.END} 任务结束。{int(LOCALDATA['token_page']) + 1}({int(LOCALDATA['token_page']) + 1} + {PAGES_TASK['step']}) / {LOCALDATA['max_page']}")
            break

        # 进入下一页
        goto(
            f"https://{LOCALDATA['token_domin']}.qq.com/cgi-bin/mail_list?folderid={LOCALDATA['folder_id']}&page={nextpage}&sid={LOCALDATA['token_sid']}&nocheckframe=true")
        time.sleep(1)

        try:
            wait_until(S('#frm').exists)
        except:
            xprint(f"\n{C.RED}等待超时{C.END}")
...
def foreach_read_mail():
    ...
    for title in LOCALDATA['title_list']:
        ...
        goto(
            f"https://{LOCALDATA['token_domin']}.qq.com/cgi-bin/frame_html?t=newwin_frame&sid={LOCALDATA['token_sid']}&url=/cgi-bin/readmail?t=readmail%26mailid={title['mailid']}%26mode=pre")
        time.sleep(1)
...
def load_attach_info(i, e, title, exmail):
    ...
    if bool(is_exmail_user):
        ...
        attach.update({'timeout': int(
            e.get_attribute('timeout') or 0)})  # 只有超大附件(bigattach="1") 有("timeout"= 0 or 1)参数。普通附件(attach="1")没有。
...
def foreach_mail_attach(title):
    ...
    scroll_down(S("#pageEnd").y)
    elements = [e.find_elements_by_tag_name('a')[0] for e in
                get_driver().find_elements_by_class_name(MAIL_SELECTOR['attach_info_class'][MAILDOMIN])]

    exmail_attach_list = {}

    if bool(is_exmail_user):
        exmail_attach_list['filename'] = [e.get_attribute('innerHTML') for e in
                                          get_driver().find_elements_by_class_name('info_name')]
        exmail_attach_list['filebyte'] = [e.text for e in get_driver().find_elements_by_class_name('info_size')]
        exmail_attach_list['filedown'] = [e.get_attribute('href') for e in
                                          get_driver().find_elements_by_class_name('attach_download')]
    ...
    for i, e in enumerate(elements, start=0):
        ...
        # 漂亮表格
        if bool(can_print_prettytable):
            PRETTY_TABLE['attach_list'].add_row(
                [f"{LOCALDATA['attach_index']:04}", attach['filename'], title['index'], title['name'], title['title'],
                 title['email'], attach['index'], attach['filebyte'], attach['filetype'], title['page'],
                 attach['timeout'], time.strftime("%Y-%m-%d %H:%M:%S", title['timestamp'])])

        # 附件类型黑名单
        if ATTACH_BACKLIST_FILETYPE != [''] and any([key in attach['filetype'] for key in ATTACH_BACKLIST_FILETYPE]):
            xprint(
                f"{C.SILVER}* {title['index']:<4}\t{LOCALDATA['attach_index']:04}: {title['email']:<24}\t{title['name']:<24}\t{attach['filename']}{C.END}")
            if bool(can_print_prettytable): PRETTY_TABLE['attach_backlist'].add_row(
                [f"{LOCALDATA['attach_index'] + 1:04}", attach['filename'], title['index'], title['name'],
                 title['title'], title['email'], attach['index'], attach['filebyte'], attach['filetype'], title['page'],
                 attach['timeout'], time.strftime("%Y-%m-%d %H:%M:%S", title['timestamp'])])
            continue

        # 附件类型白名单
        if ATTACH_WHITELIST_FILETYPE != [''] and not all(
                [key in attach['filetype'] for key in ATTACH_WHITELIST_FILETYPE]):
            xprint(
                f"{C.SILVER}* {title['index']:<4}\t{LOCALDATA['attach_index']:04}: {title['email']:<24}\t{title['name']:<24}\t{attach['filename']}{C.END}")
            if bool(can_print_prettytable): PRETTY_TABLE['attach_backlist'].add_row(
                [f"{LOCALDATA['attach_index'] + 1:04}", attach['filename'], title['index'], title['name'],
        ...
        if bool(attach['timeout']):
            xprint(
                f"{C.RED}* {title['index']:<4}\t{LOCALDATA['attach_index']:04}: {title['email']:<24}\t{title['name']:<24}\t{attach['filename']}{C.END}")
            LOCALDATA['timeout_count'] += 1
            if bool(can_print_prettytable): PRETTY_TABLE['attach_timeout'].add_row(
                [f"{LOCALDATA['attach_index'] + 1:04}", attach['filename'], title['index'], title['name'],
                 title['title'], title['email'], attach['index'], attach['filebyte'], attach['filetype'], title['page'],
                 attach['timeout'], time.strftime("%Y-%m-%d %H:%M:%S", title['timestamp'])])
            break
        ...
        if bool(can_print_attch) and not bool(can_move_folder) and not bool(can_rename_file):
            xprint(
                f"+ {title['index']:<4}\t{LOCALDATA['attach_index']:04}: {title['email']:<24}\t{title['name']:<24}\t{attach['filename']}")
...
def waitting_download(attach, title):
    ...
    while wait_time <= downloading_timeout:
        ...
        if wait_time >= 60 and not bool(wait_time % 60):
            xprint(
                f"{C.SILVER}^ {title['index']}\t{attach['index']:02}: {attach['filename']} 正在下载... 您已等待了 {wait_time / 60} 分钟。{C.END}")
...
def ready_replace_name(attach, title):
    ...
    # 重命名规则模板
    rule = {
        '{filename1}': attach['filename'].split(".")[0] if len(attach['filename'].split('.')) <= 1 else '.'.join(
            attach['filename'].split('.')[0:-1]),  # file
...
def check_file_exists(attach, title):
    ...
    filename = attach['filename'] if not bool(can_rename_file) else LOCALDATA['rule_rename'][
        int(len(LOCALDATA['rule_rename'])) - 1]
    ...
    if bool(can_move_folder):
        ...
        if check_file_matching(expectpath, filebyte):
            xprint(
                f"{C.BLUE}~ {title['index']:<4}\t{LOCALDATA['attach_index']:04}: 文件夹 {foldername} 已存在文件 {filename}，跳过本次下载。{C.END}")
            SKIP_MAIL()
...
def move_folder(foldername, newfolder=None):
    filepath = get_last_filepath()
    filename = os.path.basename(filepath)
...
def get_last_filepath(path=DOWNLOAD_FOLDER):
    ...
    files = [f for f in os.listdir(path) if
             os.path.isfile(fn(f)) and os.path.splitext(f)[1] not in ['.tmp', '.crdownload']]
...
def table_to_csv(table, filename):
    ...
    data = [tuple(filter(None, map(str.strip, splitline))) for line in raw.splitlines() for splitline in
...
def thread_webdriver():
    ...
    if S(MAIL_SELECTOR['login_frame'][MAILDOMIN]).exists(): login_qqmail() if not bool(
        is_exmail_user) else login_exmail()

    # update token_sid
    update_token_sid()

    # 获取标签列表
    if bool(can_tag_no_attach) or bool(can_tag_timeout_attach): check_tag_exists_in_list() if S(
        '#tagfoldersDiv').exists() else check_tag_exists_in_setting()
    ...
    print(
        f"包含 {len(LOCALDATA['title_list'])} 封邮件，共 {len(LOCALDATA['attach_list'])} 个附件。{tips_timeout}\n预计实际下载完成 {LOCALDATA['attach_count']} 个附件。\n\n")

    if bool(can_export_titledata_to_csv) and bool(can_load_title): table_to_csv(PRETTY_TABLE['title_list'],
                                                                                os.path.join(ROOTPATH,
                                                                                             f"_邮件统计{script_start_time}.csv"))
    if bool(can_export_attchdata_to_csv) and bool(can_load_email): table_to_csv(PRETTY_TABLE['attach_list'],os.path.join(ROOTPATH,
                                                                                             f"_附件统计{script_start_time}.csv"))
